      ...
      it('properly generate alter queries', () => {
        return current.getQueryInterface().changeColumn(Model.getTableName(), 'level_id', {
          ...
          expectsql(sql, {
            ...
            mysql: 'ALTER TABLE `sequelize_test`.`users` CHANGE `level_id` `level_id` FLOAT NOT NULL;',
      ...
      it('properly generate alter queries for foreign keys', () => {
        return current.getQueryInterface().changeColumn(Model.getTableName(), 'level_id', {
          ...
          expectsql(sql, {
            ...
            mysql: 'ALTER TABLE `sequelize_test`.`users` ADD FOREIGN KEY (`level_id`) REFERENCES `sequelize_test`.`level` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;',
